...
class CartaporteInfo (EcuInfo):
	...
	def __init__ (self, empresa, pais, distrito):
		super().__init__ (empresa, "CARTAPORTE", pais, distrito)
	...
	def extractEcuapassFields (self, docFieldsPath, analysisType="BOT"):
		...
		print (f"+++ CPI::self.pais '{self.pais}'")
	...
	def getFechaEntrega (self, fechaEntrega=None):
		...
		#-------- CPI ConfigPanel settings ------------------------------------
		self.daysFechaEntrega = Settings.getDiasEntrega (self.daysFechaEntrega)
		#----------------------------------------------------------------------
		try:
...
#		return unidades
	...
	def getInstruccionesObservaciones (self):
		...
		#------------- CPI ConfigPanel settings --------------------
		if Settings.get ("CPI", "25_Instrucciones").upper() == "NO":
			instObs ["instrucciones"] = None
		if Settings.get ("CPI", "22_Observaciones").upper() == "NO":
			instObs ["observaciones"] = None
		#-----------------------------------------------------------
		return instObs
	...
	#-- Return the "pais" for the document (pais origen before full extraction) 
	def getPaisDocumento (self):
		infoRecepcion  = self.getLocationInfo ("06_Recepcion")
		return infoRecepcion ["pais"]
