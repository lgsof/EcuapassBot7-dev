...
class ManifiestoInfo (EcuInfo):
	...
	def extractEcuapassFields (self, docFieldsPath, analysisType="BOT"):
		...
		# Internal fields
		self.ecudoc ["00_DocType"]	         = "MANIFIESTO"
		self.ecudoc ["00_DocPais"]	         = self.ecudoc ["47_Pais_Carga"]
		self.ecudoc ["00_Numero"]	         = self.ecudoc ["05_MCI"]
...
#		return info
	...
	def getMercanciaInfoManifiesto (self):
		...
		#--------- MCI ConfigPanel settings ------------------
		param = Settings.get ("MCI", "31_Mercancia_Embalaje")
		if param: docItemKeys ["embalaje"] = param 
		#-----------------------------------------------------
		...
		#--------- MCI ConfigPanel settings -----------------------------------------
		params = Settings.get ("MCI", "29_Mercancia_Descripcion")
		descripcion = mercanciaInfo ["descripcion"]
		mercanciaInfo ["descripcion"] = Settings.getDescripcion (descripcion, params)
		#----------------------------------------------------------------------------
