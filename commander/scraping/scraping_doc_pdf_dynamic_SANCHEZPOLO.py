...
def fullTest (pdf, pdfFile):
	pdf.getPdfCoordinates (pdfFile)  # main::fullTest
...
class ScrapingDocPdfDynamic_SANCHEZPOLO_Manifiesto (ScrapingDocPdfDynamic_SANCHEZPOLO):
	...
	def getDocumentBoxesFromPdf (self, boxes):
		b = boxes
		textBoxes = {}
		textBoxes ["txt0c"]     = self.boxLabel (b[0])     # DocType
		textBoxes ["txt0b"]     = self.boxLabel (b[5])     # DocPermiso
		textBoxes ["txt0a"]     = self.boxLabel (b[33])    # DocPais
		textBoxes ["txt00"]     = self.boxSimple (b[1])    # DocNumber
...
class ScrapingDocPdfDynamic_SANCHEZPOLO_Cartaporte (ScrapingDocPdfDynamic_SANCHEZPOLO):
	...
	def getDocumentBoxesFromPdf (self, boxes):
		b = boxes
		textBoxes = {}
		textBoxes ["txt0c"]     = self.boxSimple (b[0])   # DocType
		textBoxes ["txt0b"]     = self.boxSimple (b[1])   # DocPermiso | DocNumber
		textBoxes ["txt0a"]     = self.boxSimple (b[6])   # DocPais
		textBoxes ["txt00"]     = self.boxSimple (b[1])   # DocNumber
