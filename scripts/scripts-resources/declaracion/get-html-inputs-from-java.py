...
fieldKeys = list (txtFieldPairsDic.keys ())
fieldKeys.sort()
print (fieldKeys)
i = 1
for line in lines:
	if "txt27" not in line and "txt" in line and "Bounds" in line:
		print (">>> Line:", line.strip(), " Field: ", fieldKeys [i])
		key = line.split (".")[0].strip()
		res = [int (x) for x in re.findall (r"\b(\d+)\b", line)]
		boundsDic [key] = {"x":res [0], "y": res[1], "width":res[2], "value": "", "maxLines":4,
		                   "maxChars":100, "height": res [3], "fontSize": "14px", "font":"normal", 
						   "class": "input_general", "align":"left"}
		boundsDic [key]["field"] = fieldKeys [i]
		i += 1

json.dump (boundsDic, open ("output-parameters.json", "w"), indent=4, sort_keys = True)
...
with (open (f"{outputPrefix}_python_model_fields.py", "w")) as fp:
